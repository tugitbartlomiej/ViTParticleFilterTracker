...
def main():
    ...
    custom_config = {
        "num_queries": 10,  # Increase from 2 to allow more flexibility
        "bbox_cost": 4,  # Balance these coefficients
        "class_cost": 2,
        "giou_cost": 3,
        "giou_loss_coefficient": 4,
        "bbox_loss_coefficient": 5,
        "eos_coefficient": 0.5,  # Increase this to better handle empty predictions
    }
    ...
    if os.path.exists(best_model_dir) and len(os.listdir(best_model_dir)) > 0:
        ...
        try:
            ...
            model.config.focal_alpha = 0.25  # Helps with class imbalance
    ...
    visualization_samples_dir = os.path.join(output_dir, "annotation_boundingbox_validation")
    os.makedirs(visualization_samples_dir, exist_ok=True)

    selected_indices = list(range(start_idx, end_idx))[:]  # Pierwsze 50 obrazów z wybranego zakresu
