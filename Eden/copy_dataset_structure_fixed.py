...
bez kopiowania obrazów i plików TXT - tylko struktura katalogów i pliki YAML.
"""
...
def copy_directory_structure(source_dir, target_dir, include_yaml=True, exclude_images=True, exclude_txt=True):
    """
    ...
    Args:
        ...
        exclude_txt: Czy wykluczyć pliki TXT
    """
    ...
    # Rozszerzenia plików do wykluczenia
    image_extensions = {'.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.tif', '.webp', '.gif'}
    text_extensions = {'.txt'} if exclude_txt else set()
    ...
    print(f"📝 Wykluczanie plików TXT: {'✓' if exclude_txt else '✗'}")
    print()
    ...
    for root, dirs, files in os.walk(source_path):
        ...
        for file in files:
            ...
            elif exclude_txt and file_ext in text_extensions:
                should_copy = False
            ...
            if should_copy:
                ...
                if len(skipped_files) <= 20:  # Pokaż tylko pierwsze 20 pominiętych plików
    ...
    if len(skipped_files) > 20:
        print(f"\n⏭️ Pominięto łącznie {len(skipped_files)} plików (głównie obrazy i TXT)")
...
def main():
    parser = argparse.ArgumentParser(
        description='Kopiuj strukturę folderów Dataset 2025.06.06 bez obrazów i plików TXT'
    )
    ...
    parser.add_argument(
        '--include-txt',
        action='store_true',
        help='Kopiuj także pliki TXT (domyślnie: False)'
    )
    ...
    try:
        ...
        target_dir = copy_directory_structure(
            ...
            exclude_txt=not args.include_txt
        )
