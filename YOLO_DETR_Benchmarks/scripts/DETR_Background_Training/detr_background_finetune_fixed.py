...
def train(args):
    ...
    if rank == 0:
        print("Starting DETR background fine-tuning...")
    ...
    # Adjust num_workers for HPC environment to avoid deadlocks
    effective_num_workers = min(max(os.cpu_count() // world_size, 1), args.num_workers, 2)
    if rank == 0:
        print(f"Using {effective_num_workers} workers per GPU (requested: {args.num_workers})")
    ...
    train_dataloader = DataLoader(
        ...
        persistent_workers=True if effective_num_workers > 0 else False,  # Optymalizacja
        prefetch_factor=2 if effective_num_workers > 0 else None,  # Optymalizacja
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune DETR with background class - Based on detr_train_optimized.py")
