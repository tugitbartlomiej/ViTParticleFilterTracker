...
class IntelligentBackgroundFrameSelector:
    ...
    def _create_output_directories(self):
        ...
        directories = [
            ...
            'clustering_results',
            'detr_ready_dataset',
            'detr_ready_dataset/train',
            'detr_ready_dataset/val',
            'detr_ready_dataset/annotations'
        ]
    ...
    def process_videos(self, videos_dir):
        ...
        for video_file in video_files:
            ...
            extracted_frames = self.extract_frames_from_video(
                video_file, 
                self.output_dir / 'extracted_frames'
            )
            ...
            for frame_path in tqdm(extracted_frames, desc="Background detection"):
                ...
                if is_background:
                    # Copy background frame to background_frames directory
                    frame_name = Path(frame_path).name
                    background_frame_path = self.output_dir / 'background_frames' / frame_name
                    shutil.copy2(frame_path, background_frame_path)
                    
                    # Analyze content richness
                    content_score = self.analyze_content_richness(frame_path)
                    
                    video_background_frames.append({
                        'frame_path': str(background_frame_path),
            ...
            # Save video-specific analysis
            video_analysis_path = self.output_dir / 'analysis_results' / f"{video_file.stem}_analysis.json"
        ...
        complete_results_path = self.output_dir / 'analysis_results' / 'complete_detection_results.json'
    ...
    def select_significant_frames(self, background_frames):
        ...
        dino_features_file = self.output_dir / 'clustering_results' / 'dino_features.pkl'
        ...
        for frame_path in selected_frames:
            frame_name = Path(frame_path).name
            selected_frame_path = self.output_dir / 'selected_frames' / frame_name
            shutil.copy2(frame_path, selected_frame_path)
        ...
        clustering_results_path = self.output_dir / 'clustering_results' / 'clustering_results.json'
    ...
    def create_detr_dataset(self, selected_frames):
        ...
        for frame_path in train_frames:
            frame_name = Path(frame_path).name
            dst_path = self.output_dir / 'detr_ready_dataset' / 'train' / frame_name
            shutil.copy2(frame_path, dst_path)
        
        for frame_path in val_frames:
            frame_name = Path(frame_path).name
            dst_path = self.output_dir / 'detr_ready_dataset' / 'val' / frame_name
            shutil.copy2(frame_path, dst_path)
    ...
    def _create_coco_annotations(self, frame_paths, split):
        ...
        for img_id, frame_path in enumerate(frame_paths, 1):
            ...
            if img is not None:
                ...
                # Background annotation covers entire image
                annotations.append({
                    "id": img_id,
                    "image_id": img_id,
                    "category_id": 0,  # Background class
        ...
        # Save annotations
        annotations_path = self.output_dir / 'detr_ready_dataset' / 'annotations' / f'{split}_annotations.json'
        with open(annotations_path, 'w') as f:
            json.dump(coco_data, f, indent=2)
        
        print(f"Created {split} annotations: {len(images)} images, {len(annotations)} annotations")
    ...
    def save_final_report(self):
        ...
        print(f"Selected frames available in: {self.output_dir / 'selected_frames'}")
        print(f"DETR-ready dataset in: {self.output_dir / 'detr_ready_dataset'}")
...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                        default='F:/Studia/PhD_projekt/VIT/ViTParticleFilterTracker/YOLO_DETR_Benchmarks/Intelligent_Background_Selector_2025-07-17',
    ...
    parser.add_argument('--detr_threshold', type=float, default=0.3,
